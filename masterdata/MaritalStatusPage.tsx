...
const MaritalStatusPage: React.FC = () => {
    ...
    const [editingItem, setEditingItem] = useState<Partial<MaritalStatus> | null>(null);
    const [draggedItemId, setDraggedItemId] = useState<number | null>(null);
    const [isWarningModalOpen, setIsWarningModalOpen] = useState(false);
    const [itemToAction, setItemToAction] = useState<MaritalStatus | null>(null);
    const [dependentItems, setDependentItems] = useState<{ name: string; type: string }[]>([]);

    const canCreate = true;
    const canModify = true;
    const noun = "Marital Status";
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            addToast("Failed to load marital statuses.", "error");
        } finally {
            setIsLoading(false);
        }
    }, [addToast]);
    ...
    if (isLoading) {
        return <div className="p-8 text-center text-slate-500 dark:text-slate-400">Loading marital statuses...</div>;
    }

    return (
        <div className="w-full h-full flex flex-col">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-6">Manage Marital Statuses</h2>
            <div className="flex items-center justify-between mb-4">
                <SearchBar searchQuery={searchQuery} onSearchChange={setSearchQuery} placeholder={`Search ${noun}es...`} className="max-w-md" />
